...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` — still RESOLVED. No newer `deploy-drift.yml` run since the prior audit: still 33242624209 (2026-08-29T08:12:53Z, `success`, ~16h49m old — within the 26h staleness window) — prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 25th consecutive audit) | 2026-08-30T01:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — still healthy. Most recent completed uptime-monitor run (33284729564) succeeded at 2026-08-30T01:02:13Z, effectively concurrent with this audit — well within the 30-min freshness window. Issue #70 remains open for human close. Direct egress still blocked | 2026-08-30T01:02Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-30T01:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-30T01:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
