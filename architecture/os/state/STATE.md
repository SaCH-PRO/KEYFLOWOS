...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — still RESOLVED: most recent completed `deploy-drift.yml` run is still 33486212863 (2026-09-01T08:16:42Z, `success`) — no newer run since the 01:01Z audit cited it. ~22h46m old, within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 38th consecutive audit) | 2026-09-02T07:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — RESOLVED: most recent completed uptime-monitor run (33601174610) succeeded at 2026-09-02T06:56:50Z, ~6m old — well within the 30-min freshness window. Prod healthy. The 01:01Z cycle's `uptime-monitor.stale` warn (issue #70) did not recur this cycle | 2026-09-02T07:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-02T07:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-02T07:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
