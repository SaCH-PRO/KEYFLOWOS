...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `deploy-drift.yml` run 32825752514 (2026-08-25T08:16Z, `failure`) is still the most recent completed run — no new drift check has fired since (daily cadence, next expected ~2026-08-26T08:00Z). prod (`ea9a21fc6dca`) remains 64 commits / ~13 days behind main (oldest missing commit 2026-08-12T00:38:24Z), over the 7-day threshold → `warn` `commit-drift`, now unresolved across 4 consecutive audit cycles (2026-08-25 13:03Z, 19:02Z; 2026-08-26 01:02Z, 07:04Z) → already escalated to open issue #69, no new issue filed. Direct egress still blocked (403 on CONNECT keyflowos.com:443) | 2026-08-26T07:04Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | healthy via CI fallback — uptime-monitor run 32940420108, success at 2026-08-26T06:57:13Z (~7 min before this run); direct egress still blocked | 2026-08-26T07:04Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-26T07:04Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-26T07:04Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
