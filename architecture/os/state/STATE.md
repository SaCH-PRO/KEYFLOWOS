...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run 34203590374 (2026-09-08T08:16:13Z, `success`), head_sha `11fc6691` exactly matches current `origin/main` HEAD — zero drift. ~4h46m old, within the 26h staleness window. Direct egress still blocked (403 on CONNECT keyflowos.com:443; 63rd consecutive audit) | 2026-09-08T13:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34229163821) succeeded at 2026-09-08T12:58:31Z, ~3min44s old — well within the 30-min freshness window. Prod healthy, confirmed indirectly | 2026-09-08T13:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-08T13:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-08T13:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
