...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `deploy-drift.yml` run 32825752514 (2026-08-25T08:16Z, `failure`) is still the most recent completed run — no new drift check has fired since. prod (`ea9a21fc6dca`) remains 64 commits / ~13 days behind main (oldest missing commit 2026-08-12T00:38:24Z), over the 7-day threshold → `warn` `commit-drift`, now unresolved across 2 consecutive audit cycles (13:03Z, 19:02Z) → escalated to issue #69. Direct egress still blocked (403 on CONNECT keyflowos.com:443) | 2026-08-25T19:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | healthy via CI fallback — uptime-monitor run 32885830406, success at 2026-08-25T18:47:34Z (~15 min before this run); direct egress still blocked | 2026-08-25T19:02Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-25T19:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-25T19:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
