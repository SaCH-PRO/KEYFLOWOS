...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` — still RESOLVED. Most recent completed `deploy-drift.yml` run: 33301063267 (2026-08-30T08:13:37Z, `success`, ~10h48m old — within the 26h staleness window) — prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 28th consecutive audit) | 2026-08-30T19:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — still healthy. Most recent completed uptime-monitor run (33329459976) succeeded at 2026-08-30T18:56:11Z, ~5m before this audit — well within the 30-min freshness window. Issue #70 remains open for human close. Direct egress still blocked | 2026-08-30T19:02Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-30T19:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-30T19:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
