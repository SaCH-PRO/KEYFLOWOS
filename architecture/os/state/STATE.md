...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — still RESOLVED: most recent completed `deploy-drift.yml` run is now 33607833269 (2026-09-02T08:16:23Z, `success`), a newer run than the 07:02Z audit cited (33486212863). ~4h45m old, within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 39th consecutive audit) | 2026-09-02T13:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — RESOLVED: most recent completed uptime-monitor run (33632744525) succeeded at 2026-09-02T12:56:11Z, ~5m45s old — well within the 30-min freshness window. Prod healthy | 2026-09-02T13:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-02T13:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-02T13:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
