...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | New `deploy-drift.yml` run since the prior audit — run 33067311510 (2026-08-27T11:25:36Z, `failure`). prod (`ea9a21fc6dca`) is now 75 commits / ~15 days behind main (oldest missing commit 2026-08-12T00:38:24Z, was 70/~14d), over the 7-day threshold → `warn` `commit-drift`, now unresolved across 9 consecutive audit cycles (2026-08-25 13:03Z, 19:02Z; 2026-08-26 01:02Z, 07:04Z, 13:33Z, 19:02Z; 2026-08-27 01:04Z, 07:03Z, 13:04Z) → already escalated to open issue #69, no new issue filed. Direct egress still blocked (403 on CONNECT keyflowos.com:443) | 2026-08-27T13:04Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (33074213973) succeeded at 2026-08-27T12:55:34Z, 9 min before this audit (within the 30-min freshness window; recovered from the prior cycle's 2nd-consecutive `uptime-monitor.stale`, escalated to issue #70, which stands open for the human to close). Direct egress still blocked | 2026-08-27T13:04Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-27T13:04Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-27T13:04Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
