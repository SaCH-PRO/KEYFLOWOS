...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run 34328157919 (2026-09-09T08:15:49Z, `success`), head_sha `96e05439` — zero drift. ~22h46m old, still within the 26h staleness window (no newer run since last cycle; daily workflow, next expected ~2026-09-10T08:1xZ). Direct egress still blocked (403 on CONNECT keyflowos.com:443; 70th consecutive audit) | 2026-09-10T07:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34447683663) succeeded at 2026-09-10T06:58:39Z, ~3min old — well within the 30-min freshness window, against head_sha `27d525c4` (current origin/main HEAD, exact match). Prod healthy, confirmed indirectly | 2026-09-10T07:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-10T07:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-10T07:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
