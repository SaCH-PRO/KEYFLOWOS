...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run 34328157919 (2026-09-09T08:15:49Z, `success`), head_sha `96e05439` — zero drift. ~16h46m old, well within the 26h staleness window (no newer run since last cycle; daily workflow). Direct egress still blocked (403 on CONNECT keyflowos.com:443; 69th consecutive audit) | 2026-09-10T01:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34422695241) succeeded at 2026-09-10T00:46:16Z, ~15min26s old — well within the 30-min freshness window, against head_sha `57cc1a1d` (current origin/main HEAD). Prod healthy, confirmed indirectly | 2026-09-10T01:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-10T01:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-10T01:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
