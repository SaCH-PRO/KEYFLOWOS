...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run is now 33954666486 (2026-09-05T08:13:14Z, `success`) — a new run since the last four audits (which all saw stale run 33852601893 from 2026-09-04T08:15:56Z); ~4h48m old, well within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close (historical). Direct egress still blocked (403 on CONNECT keyflowos.com:443, confirmed by curl exit 56 CONNECT tunnel failure; 51st consecutive audit) | 2026-09-05T13:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (33967502166) succeeded at 2026-09-05T12:57:25Z, ~4m old — well within the 30-min freshness window. Prod healthy | 2026-09-05T13:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-05T13:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-05T13:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
