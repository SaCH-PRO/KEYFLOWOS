...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — still RESOLVED. Most recent completed `deploy-drift.yml` run is still 33372254273 (2026-08-31T08:17:58Z, `success`) — no newer run since the 19:01Z audit; ~16h44m old, within the 26h staleness window (unchanged for 3 consecutive audits). Prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 33rd consecutive audit) | 2026-09-01T01:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci`. Most recent completed uptime-monitor run (33455283287) succeeded at 2026-09-01T00:34:17Z, ~27m44s before this audit — within the 30-min freshness window. Prod healthy, confirmed indirectly. Direct egress still blocked | 2026-09-01T01:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-01T01:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-01T01:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
