...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` — still RESOLVED. New `deploy-drift.yml` run since the prior audit: 33242624209 (2026-08-29T08:12:53Z, `success`, ~4h50m old — within the 26h staleness window) — prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 23rd consecutive audit) | 2026-08-29T13:03Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — still healthy. Most recent completed uptime-monitor run (33253751852) succeeded at 2026-08-29T12:56:33Z, ~6m before this audit — well within the 30-min freshness window. Issue #70 remains open for human close. Direct egress still blocked | 2026-08-29T13:03Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-29T13:03Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-29T13:03Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
