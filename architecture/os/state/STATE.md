...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | No new `deploy-drift.yml` run since the prior audit — still run 33067311510 (2026-08-27T11:25:36Z, `failure`); next run expected ~2026-08-28T08:00Z (daily cron). prod (`ea9a21fc6dca`) still 75 commits / ~15 days behind main (oldest missing commit 2026-08-12T00:38:24Z), over the 7-day threshold → `warn` `commit-drift`, now unresolved across 10 consecutive audit cycles (2026-08-25 13:03Z, 19:02Z; 2026-08-26 01:02Z, 07:04Z, 13:33Z, 19:02Z; 2026-08-27 01:04Z, 07:03Z, 13:04Z, 19:02Z) → already escalated to open issue #69, no new issue filed. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 16th consecutive audit) | 2026-08-27T19:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `warn` `uptime-monitor.stale` — most recent completed uptime-monitor run (33094567511) succeeded at 2026-08-27T16:41:59Z, 2h20m before this audit — past the 30-min freshness window despite the `*/5 * * * *` cron. Recurrence: last audit (13:04Z) was fresh (9 min); watcher went dark again this cycle. Issue #70 (from the earlier 2-consecutive occurrence) is still open and already covers this check id — no new issue filed. Direct egress still blocked | 2026-08-27T19:02Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-27T19:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-27T19:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
