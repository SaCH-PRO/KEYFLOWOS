...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run is still 33954666486 (2026-09-05T08:13:14Z, `success`) — same run as last audit, expected: deploy-drift.yml is a daily 08:00 UTC schedule and no new run has fired since; ~10h49m old, well within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close (historical). Direct egress still blocked (403 on CONNECT keyflowos.com:443, confirmed by curl exit 56 CONNECT tunnel failure; 52nd consecutive audit) | 2026-09-05T19:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (33985526731) succeeded at 2026-09-05T18:55:09Z, ~7m old — well within the 30-min freshness window. Prod healthy | 2026-09-05T19:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-05T19:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-05T19:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
