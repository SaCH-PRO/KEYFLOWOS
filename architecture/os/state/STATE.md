...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | No new `deploy-drift.yml` run since the prior audit — most recent completed run is still 32946923369 (2026-08-26T08:16:38Z, `failure`). prod (`ea9a21fc6dca`) remains 70 commits / ~14 days behind main (oldest missing commit 2026-08-12T00:38:24Z), over the 7-day threshold → `warn` `commit-drift`, now unresolved across 7 consecutive audit cycles (2026-08-25 13:03Z, 19:02Z; 2026-08-26 01:02Z, 07:04Z, 13:33Z, 19:02Z; 2026-08-27 01:04Z) → already escalated to open issue #69, no new issue filed. Direct egress still blocked (403 on CONNECT keyflowos.com:443) | 2026-08-27T01:04Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `warn` `uptime-monitor.stale` — most recent completed uptime-monitor run (33026815550) succeeded at 2026-08-27T00:25:58Z, but that is 38+ min before this audit (outside the 30-min freshness window; cron is */5 * * * *, so the gap itself is anomalous, likely a GitHub Actions scheduling delay). First occurrence, not yet escalated. Direct egress still blocked | 2026-08-27T01:04Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-27T01:04Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-27T01:04Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
