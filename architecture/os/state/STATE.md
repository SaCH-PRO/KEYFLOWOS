...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` — still RESOLVED. No new `deploy-drift.yml` run since the prior audit; most recent remains 33168875790 (2026-08-28T11:53:31Z, `success`) — prod (`0ac8e5392867`) still only 3 commits / ~0 days behind main, well within the 7-day/200-commit threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 21st consecutive audit) | 2026-08-29T01:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — **RESOLVED this cycle.** Most recent completed uptime-monitor run (33224562361) succeeded at 2026-08-29T00:47:21Z, ~13m before this audit — back within the 30-min freshness window. Cadence recovered to ~6min/run (00:47:21Z, 00:00:34Z, 23:54:27Z, 23:48:40Z, 23:42:52Z), ending the `uptime-monitor.stale` streak (5 consecutive occurrences, 2026-08-27T19:02Z–2026-08-28T19:03Z). Issue #70 remains open for human close. Direct egress still blocked | 2026-08-29T01:01Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-29T01:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-29T01:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
