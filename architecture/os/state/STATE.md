...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run still 33852601893 (2026-09-04T08:15:56Z, `success`), same run seen last three audits (no newer run produced since; next expected ~08:16Z today); ~22h45m old, still within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close (historical). Direct egress still blocked (403 on CONNECT keyflowos.com:443, confirmed by curl -v CONNECT failure; 50th consecutive audit) | 2026-09-05T07:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (33951153859) succeeded at 2026-09-05T06:55:49Z, ~6m old — well within the 30-min freshness window. Prod healthy | 2026-09-05T07:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-05T07:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-05T07:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
