...
| Measure | Value | Prev (2026-09-11) | Command |
|---|---:|---:|---|
...
| Server tests passing | **3,658 in 373 files, 0 skipped** (unit config) | 3,658 in 373 files, 0 skipped | `cd apps/server && pnpm test:unit` (= `vitest -c vitest.unit.config.ts`; do NOT run bare `npx vitest run` — needs DB, false-reds as skips, see truth.md); seed `1789197228567` |
...
`node scripts/os/ledger-sizes.mjs` (2026-09-12) again reports
...
movement**. `git diff --name-only d91b935b..HEAD` (last truth commit →
...
|---|---|---|
| architecture/{module,route,event,capability,data-ownership}-registry.yaml | `node scripts/architecture/generate-registries.js` | 2026-09-12 (no real drift — only `generated:` date changed on all five files; `git diff` confirms zero non-date-line changes) |
| docs/architecture/capability-map/* + apps/server/src/modules/ai/capability-map/capability-map.seed.ts | `node docs/architecture/capability-map/generate.js` | 2026-09-12 (byte-identical output to 2026-09-11 — no drift; 286 flowTools, 207 cortexCapabilities, 12 domains, 32 targets (15 active/17 planned) unchanged) |
| architecture/inventory.json | `python3 .agents/skills/codebase-architect/scripts/inventory.py > architecture/inventory.json` | 2026-09-12 (small, fully-attributable delta: total_files 4323→4328 (+5), total_lines 1,043,270→1,043,769 (+499) — the real accumulated diff since the last truth commit `d91b935b` is exactly 5 new journal files: `2026-09-11-audit-13.md` (125), `2026-09-11-audit-19.md` (77), `2026-09-11-truth.md` (119, the prior cycle's own journal, written after that cycle's registry regeneration), `2026-09-12-audit-01.md` (94), `2026-09-12-audit-07.md` (84) = 499 lines / 5 files, verified line-by-line against `d91b935b`'s committed inventory.json; the `.claude/coordination/sessions/<uuid>.json` rename is a net-zero swap (one session file for another, same shape); STATE.md's own recorded size moved 9289→9800 bytes reflecting audit-cycle §Runtime writes since `d91b935b`, expected under the write matrix; no unexplained movement) |
| architecture/dependencies.json | `python3 .agents/skills/codebase-architect/scripts/dependency_scan.py > architecture/dependencies.json` | 2026-09-12 (byte-identical to the committed 2026-09-11 file — no drift from this run) |

## Runtime (written by audit cycle only)

| Check | Last result | At | Command |
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `warn` `commit-drift`, **still open on issue [#73](https://github.com/SaCH-PRO/KEYFLOWOS/issues/73)** — most recent completed `deploy-drift.yml` run is still 34578300288 (2026-09-11T08:15:36Z, `failure`; ~22h46m old, within the 26h staleness window so still authoritative; no newer run has fired, it's a daily workflow) — prod=`2c3da5d4569e` is still 44 commits / ~8 days behind main (oldest missing commit 2026-09-03T06:32:39Z), over the 7-day `DRIFT_DAYS` threshold. 4th consecutive audit run reading this unresolved condition (1st 13:02Z, 2nd 19:02Z on 2026-09-11 — escalated to #73 at the 2nd, 3rd 01:02Z on 2026-09-12); this run just confirms #73 is still open and accurate, no new issue. Open issue #69 remains a distinct, already-resolved earlier episode. Direct egress still blocked (403/connect_rejected on CONNECT keyflowos.com:443; 78th consecutive audit) | 2026-09-12T07:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34679597303) succeeded at 2026-09-12T07:00:35Z, ~1-2 min old — well within the 30-min freshness window, cadence back to normal (5-13 min gaps in the runs immediately before it). Prod healthy, confirmed indirectly. Previous cycle's `uptime-monitor.stale` warn (issue #70) does NOT recur this cycle | 2026-09-12T07:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-12T07:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-12T07:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
