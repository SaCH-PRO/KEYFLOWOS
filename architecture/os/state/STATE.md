...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — still RESOLVED: most recent completed `deploy-drift.yml` run is still 33486212863 (2026-09-01T08:16:42Z, `success`) — no newer run since the 19:01Z audit cited it. ~16h46m old, within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 37th consecutive audit) | 2026-09-02T01:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `warn` `uptime-monitor.stale`. Most recent completed uptime-monitor run (33575407040) succeeded at 2026-09-02T00:27:29Z but is ~35m old — outside the 30-min freshness window (cron is `*/5 * * * *`). No newer completed/in_progress/queued run (checked twice, ~40s apart); workflow `state: active`. Prod health NOT confirmed this cycle via either path. Same condition as issue #70 (2026-08-27, still open — self-resolved for a month, now recurred); commented with new evidence rather than opening a duplicate. First occurrence this streak, so no fresh 2-consecutive-runs escalation | 2026-09-02T01:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-02T01:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-02T01:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
