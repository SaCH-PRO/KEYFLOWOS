...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `warn` `commit-drift` — most recent completed `deploy-drift.yml` run 34578300288 (2026-09-11T08:15:36Z, `failure`), head_sha `d91b935b` (the checkout; the job's own drift check ran against prod) — prod=`2c3da5d4569e` is 44 commits / ~8 days behind main (oldest missing commit 2026-09-03T06:32:39Z), over the 7-day `DRIFT_DAYS` threshold. 1st occurrence this recurrence (prior audit at 07:02Z read `info`/success against `d3185b04`) — does not yet meet the 2-consecutive-audit-run escalation to an issue. Open issue #69 tracks an earlier, since-resolved drift episode (2026-08-25, prod `ea9a21fc6dca`); this is a new recurrence, not a duplicate. Direct egress still blocked (403/connect_rejected on CONNECT keyflowos.com:443; 75th consecutive audit) | 2026-09-11T13:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34601512473) succeeded at 2026-09-11T12:56:05Z, ~6m old — well within the 30-min freshness window, against head_sha `d91b935b` (current origin/main HEAD, exact match). Prod healthy, confirmed indirectly | 2026-09-11T13:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-11T13:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-11T13:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
