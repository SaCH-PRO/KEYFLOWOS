...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run 34099867050 (2026-09-07T08:18:04Z, `success`) — same run already cited in the 13:02Z audit, no new daily run yet; ~10h44m old, still within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close (historical, unrelated to this cycle's fresh reading). Direct egress still blocked (403 on CONNECT keyflowos.com:443, confirmed by verbose curl CONNECT-tunnel 403; 60th consecutive audit) | 2026-09-07T19:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34153706682) succeeded at 2026-09-07T18:57:36Z, ~4m5s old — within the 30-min freshness window. Prod healthy via CI fallback; no recurrence of the #70 staleness condition this cycle | 2026-09-07T19:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-07T19:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-07T19:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
