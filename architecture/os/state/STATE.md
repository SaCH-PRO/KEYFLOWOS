...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `warn` `commit-drift`, **still open on issue [#73](https://github.com/SaCH-PRO/KEYFLOWOS/issues/73)** — most recent completed `deploy-drift.yml` run is still 34578300288 (2026-09-11T08:15:36Z, `failure`; ~16h46m old, within the 26h staleness window so still authoritative; no newer run has fired, it's a daily workflow) — prod=`2c3da5d4569e` is still 44 commits / ~8 days behind main (oldest missing commit 2026-09-03T06:32:39Z), over the 7-day `DRIFT_DAYS` threshold. 3rd consecutive audit run reading this unresolved condition (1st 13:02Z, 2nd 19:02Z on 2026-09-11 — escalated to #73 at the 2nd); this run just confirms #73 is still open and accurate, no new issue. Open issue #69 remains a distinct, already-resolved earlier episode. Direct egress still blocked (403/connect_rejected on CONNECT keyflowos.com:443; 77th consecutive audit) | 2026-09-12T01:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `warn` `uptime-monitor.stale` — most recent completed uptime-monitor run (34661690515) succeeded at 2026-09-12T00:27:24Z but that is now ~35 min old, outside the 30-min freshness window, and no run has fired since despite the `*/5 * * * *` cron (no in-progress/queued run either) — the watcher itself is dark, so prod health could not be confirmed via either path this cycle. Pre-existing open issue [#70](https://github.com/SaCH-PRO/KEYFLOWOS/issues/70) already covers this exact check (recurring since 2026-08-27) — no new issue opened, just re-confirmed unresolved | 2026-09-12T01:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-12T01:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-12T01:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
