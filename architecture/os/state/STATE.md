...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` — still RESOLVED. No new `deploy-drift.yml` run since the prior audit; most recent remains 33168875790 (2026-08-28T11:53:31Z, `success`) — prod (`0ac8e5392867`) still only 3 commits / ~0 days behind main, well within the 7-day/200-commit threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 20th consecutive audit) | 2026-08-28T19:03Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `warn` `uptime-monitor.stale` — most recent completed uptime-monitor run (33184797382) succeeded at 2026-08-28T15:22:12Z, **3h40m** before this audit — past the 30-min freshness window despite the `*/5 * * * *` cron, and zero runs of any status recorded since (largest gap in the current streak; prior worst was 1h49m). 5th consecutive occurrence (2026-08-27 19:02Z, 2026-08-28 01:02Z, 07:03Z, 13:04Z, now 19:03Z). Issue #70 is still open and already covers this check id — no new issue filed. Direct egress still blocked | 2026-08-28T19:03Z | `curl -s -o /dev/null -w '%{http_code}' $PROD/api/readyz` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-28T19:03Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-28T19:03Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
