...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run 34099867050 (2026-09-07T08:18:04Z, `success`) — same run already cited in the 13:02Z, 19:01Z (09-07) and 01:02Z (09-08) audits, no new daily run yet; ~22h44m old, still within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close (historical, unrelated to this cycle's fresh reading). Direct egress still blocked (403 on CONNECT keyflowos.com:443, confirmed by verbose curl CONNECT-tunnel 403; 62nd consecutive audit) | 2026-09-08T07:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34197134942) succeeded at 2026-09-08T06:59:16Z, ~3min14s old — well within the 30-min freshness window. Prod healthy, confirmed indirectly. Previous cycle's `uptime-monitor.stale` warn (issue #70) does NOT recur this cycle | 2026-09-08T07:02Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-08T07:02Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-08T07:02Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
