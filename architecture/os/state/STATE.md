...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` `commit-drift` — most recent completed `deploy-drift.yml` run remains 34021268073 (2026-09-06T08:13:46Z, `success`) — no new run since the 13:01Z audit; ~10h48m old, still within the 26h staleness window. Prod current/within threshold. Issue #69 remains open for human close (historical, unrelated to this cycle's fresh reading). Direct egress still blocked (403 on CONNECT keyflowos.com:443, confirmed by verbose curl CONNECT-tunnel 403; 56th consecutive audit) | 2026-09-06T19:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `info` `prod.health.via-ci` — most recent completed uptime-monitor run (34053408485) succeeded at 2026-09-06T18:58:50Z, ~2m39s old — well within the 30-min freshness window. Prod healthy, confirmed indirectly via CI | 2026-09-06T19:01Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-09-06T19:01Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-09-06T19:01Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
