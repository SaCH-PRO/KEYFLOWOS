...
|---|---|---|---|
| /healthz.commit == origin/main HEAD | `info` — still RESOLVED. Most recent completed `deploy-drift.yml` run: 33301063267 (2026-08-30T08:13:37Z, `success`, ~16h51m old — within the 26h staleness window) — prod current/within threshold. Issue #69 remains open for human close. Direct egress still blocked (403 on CONNECT keyflowos.com:443, 29th consecutive audit) | 2026-08-31T01:04Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow deploy-drift.yml --limit 3 --json conclusion,status,createdAt` |
| /readyz | `warn` `uptime-monitor.stale` — CI watcher went dark. Most recent completed uptime-monitor run (33344449390) succeeded at 2026-08-31T00:23:40Z but that's ~40m before this audit, outside the 30-min freshness window (no queued/in_progress run either); prod health not confirmed this cycle (not a confirmed outage — last ping was clean). Not 2 consecutive audits with this finding (prior cycle 19:02Z was fresh), so no new issue — already tracked by open issue #70. Direct egress still blocked | 2026-08-31T01:04Z | `gh run list --repo SaCH-PRO/KEYFLOWOS --workflow uptime-monitor.yml --limit 5 --json conclusion,status,createdAt,databaseId` |
| /healthz/events queue depth | unavailable — CI fallback exposes no queue metrics; direct egress still blocked | 2026-08-31T01:04Z | `curl -s $PROD/api/healthz/events` |
| route-parity oracle (6 ledgered paths) | skipped — CI-fallback path has no route-parity substitute (oracle needs direct egress); not filed as blind this run, per playbook | 2026-08-31T01:04Z | `node scripts/os/probe-routes.mjs --base $PROD/api --routes <ledger-paths>` |
