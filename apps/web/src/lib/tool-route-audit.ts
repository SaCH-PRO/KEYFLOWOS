/**
 * Every FlowTool in the registry must declare `manualEquivalentRoute` pointing
 * at the in-app screen where a human can do the same thing without KEY. That is
 * the "AI is optional, never mandatory" principle, enforced.
 *
 * This module is the ANALYSIS. It reads, it decides, it returns findings — it
 * never prints and it never exits. Two callers consume it:
 *
 *   scripts/check-tool-routes.ts          the CLI: prints, sets an exit code
 *   src/lib/__tests__/tool-routes.spec.ts the gate: runs inside `pnpm test:ci`
 *
 * It was a standalone script until 2026-08-09, and that was the whole problem.
 * ci-cd.yml runs test:ci, build and audit; it never ran this. So the gate fired
 * only when somebody remembered to type the command — and when the fork-close
 * merge left twelve tools pointing at routes that did not survive the union,
 * the pipeline stayed green throughout. A gate outside CI catches things after
 * they ship.
 */
...
export interface ToolRouteFindings {
  tools: AuditTool[];
  /** Tools whose family means they CHANGE something. */
  writeTools: AuditTool[];
  /** No manualEquivalentRoute, or one that is not a path. */
  missing: AuditTool[];
  /** Declares a route with no page behind it. */
  unresolved: AuditTool[];
  /** A write tool pointing at a screen in another domain. */
  crossDomain: AuditTool[];
  /** Declares a route next.config redirects elsewhere. */
  redirectedAway: AuditTool[];
  /** A write tool pointing at a screen a human cannot write from. */
  noManualWrite: AuditTool[];
}
...
export function defaultAuditPaths(): Required<AuditPaths> {
  return {
    registryPath: path.resolve(__dirname, '../../../server/src/modules/ai/flow-tool-registry.ts'),
...
/**
 * Run every check and return what failed. Throws only if the registry cannot be
 * read at all — that is a broken invocation, not a finding.
 */
export async function auditToolRoutes(paths: AuditPaths = {}): Promise<ToolRouteFindings> {
  const { registryPath, appDir, appRoot } = { ...defaultAuditPaths(), ...paths };

  let mod: { FLOW_TOOLS?: AuditTool[] };
  try {
    mod = (await import(pathToFileURL(registryPath).href)) as { FLOW_TOOLS?: AuditTool[] };
  } catch (err) {
    throw new Error(`Failed to load the tool registry at ${registryPath}: ${(err as Error).message}`);
  }

  const tools = mod.FLOW_TOOLS;
  if (!Array.isArray(tools)) {
    throw new Error(`FLOW_TOOLS export not found in ${registryPath}`);
  }
  ...
  return { tools, writeTools, missing, unresolved, crossDomain, redirectedAway, noManualWrite };
}

/** Where a tool's declared route ends up after next.config redirects. */
export function resolveDeclaredRoute(route: string, appRoot: string): string {
  return followConfigRedirects(route, nextConfigRedirects(appRoot));
}

/** One line per finding, shared by the CLI and the spec's failure messages. */
export function describeFindings(f: ToolRouteFindings): string[] {
  const lines: string[] = [];
  const list = (tools: AuditTool[], render: (t: AuditTool) => string) =>
    tools.map((t) => `    - ${render(t)}`);

  if (f.missing.length) {
    lines.push(`${f.missing.length} of ${f.tools.length} tool(s) missing manualEquivalentRoute:`);
    lines.push(...list(f.missing, (t) => `${t.name} (family=${t.family})`));
    lines.push('  Every tool must declare an in-app screen where a human can do the same thing.');
  }
  if (f.unresolved.length) {
    lines.push(`${f.unresolved.length} tool(s) point at a route with no page:`);
    lines.push(...list(f.unresolved, (t) => `${t.name} -> ${t.manualEquivalentRoute}`));
    lines.push('  Point each at a screen that exists under apps/web/src/app.');
  }
  if (f.crossDomain.length) {
    lines.push(`${f.crossDomain.length} write tool(s) point outside their own domain:`);
    lines.push(...list(f.crossDomain, (t) => `${t.name} -> ${t.manualEquivalentRoute}`));
    lines.push('  Point the tool at its own screen, or add a CROSS_DOMAIN_ROUTES entry');
    lines.push('  saying why this destination is where a person really does this.');
  }
  if (f.redirectedAway.length) {
    lines.push(`${f.redirectedAway.length} tool(s) declare a route next.config redirects away:`);
    lines.push(...list(f.redirectedAway, (t) => `${t.name} -> ${t.manualEquivalentRoute}`));
    lines.push('  Open the destination. If it is the same screen, add a REDIRECTED_BY_DESIGN');
    lines.push('  entry; if it is not, the manual equivalent does not exist.');
  }
  if (f.noManualWrite.length) {
    lines.push(`${f.noManualWrite.length} write tool(s) point at a screen a human cannot write from:`);
    lines.push(...list(f.noManualWrite, (t) => `${t.name} (${t.family}) -> ${t.manualEquivalentRoute}`));
    lines.push('  KEY must be optional, never mandatory: anything it can change, a person must');
    lines.push('  be able to change too. Build the screen, repoint the tool, or add a reviewed');
    lines.push('  MANUAL_PARITY_EXEMPTIONS entry with the reason.');
  }
  return lines;
}

/** The one-line summary the CLI prints on success. */
export function summarise(f: ToolRouteFindings): string {
  return (
    `${f.tools.length - f.missing.length}/${f.tools.length} tools declare manualEquivalentRoute, ` +
    `${f.tools.length - f.unresolved.length}/${f.tools.length} resolve to a real page, ` +
    `${f.writeTools.length - f.noManualWrite.length}/${f.writeTools.length} write tools have manual parity, ` +
    `${f.writeTools.length - f.crossDomain.length}/${f.writeTools.length} point inside their own domain.`
  );
}
...
function assertParityCheckWorks(appDir: string, appRoot: string): void {
  ...
  if (problems.length > 0) {
    // Throws rather than exits: this is the check failing its own self-test, so
    // it is a broken invocation, not a finding about the tools. Both callers
    // surface it as an error — the CLI with exit code 2, the spec as a failure.
    throw new Error(
      'the manual-parity check is broken:\n' +
        problems.map((p) => `  - ${p}`).join('\n') +
        '\nFix the check, or pick fixtures whose behaviour still matches the comment above.',
    );
  }
}
