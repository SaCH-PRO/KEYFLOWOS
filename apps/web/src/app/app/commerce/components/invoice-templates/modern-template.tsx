    ...
    <div className="bg-white text-gray-900 rounded-xl overflow-hidden shadow-lg">
          ...
          <div className="flex items-center gap-4">
            {business.logoUrl ? (
              <img
                ...
                alt={business.name}
                className="h-16 w-16 object-contain rounded-xl bg-white/20 p-1"
              />
            ) : (
              <div className="h-16 w-16 rounded-xl bg-white/20 flex items-center justify-center text-white font-bold text-2xl backdrop-blur-sm">
            ...
            <div>
              ...
              {business.tagline && (
                <p className="text-sm text-white/70 mt-0.5">{business.tagline}</p>
              )}
            </div>
          </div>
          <div className="text-right">
            <h1 className="text-3xl font-black tracking-wider">{docLabel}</h1>
            <p className="text-sm text-white/80 mt-1 font-medium">#{data.number}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-8 py-6">
        <div className="grid grid-cols-3 gap-4 mb-8 -mt-8">
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-100 p-4 text-center">
            <p className="text-[10px] uppercase tracking-wider text-gray-400 mb-1">Total Due</p>
            <p className="text-sm font-bold" style={{ color: pc }}>{fmt(data.total, data.currency)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            ...
            <div className="space-y-1 text-sm text-gray-600">
              ...
              {business.address && (
                <p className="flex items-center gap-1"><MapPin className="w-3 h-3 shrink-0" />{business.address}{business.city ? `, ${business.city}` : ""}</p>
              )}
              {business.phone && (
                <p className="flex items-center gap-1"><Phone className="w-3 h-3 shrink-0" />{business.phone}</p>
              )}
              {business.email && (
                <p className="flex items-center gap-1"><Mail className="w-3 h-3 shrink-0" />{business.email}</p>
              )}
              {business.website && (
        ...
        <div className="flex justify-end mb-6">
          <div
            className="w-72 rounded-xl p-5 space-y-2"
          ...
          >
            ...
            <div
              className="flex justify-between items-center text-xl font-bold pt-3 mt-2 border-t"
      ...
      <div
        className="px-8 py-4 text-center text-white text-xs"
        style={{ background: `linear-gradient(135deg, ${pc} 0%, ${sc} 100%)` }}
      >
