...
import { LegalDisclaimerModal } from "../../onboarding/components/LegalDisclaimerModal";

interface LegalDocumentPackProps {
...
export function LegalDocumentPack({ blueprint }: LegalDocumentPackProps) {
                  ...
                  {sources.map((doc) => (
                    <li key={doc.instanceId}>
                      <Link
                        ...
                        className="flex items-center gap-3 p-3 rounded-lg hover:bg-muted/50 transition-colors group border border-transparent hover:border-[hsl(var(--kf-border)/0.5)]"
                      >
