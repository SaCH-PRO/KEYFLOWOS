...
export function LegalChecklist({ items, title = "Compliance checklist" }: LegalChecklistProps) {
  const [localItems, setLocalItems] = useState<ComplianceItem[]>(() =>
    [...items].sort((a, b) => {
  ...
  const doneCount = useMemo(
    () => localItems.filter((i) => i.status === "DONE" || i.status === "NOT_APPLICABLE").length,
    [localItems],
  );
  const progress = localItems.length ? Math.round((doneCount / localItems.length) * 100) : 0;
    ...
    >
      ...
      {localItems.length === 0 ? (
        <p className="text-sm text-muted-foreground py-2">
          No compliance items yet. Complete your business blueprint to generate them.
        </p>
      ) : (
