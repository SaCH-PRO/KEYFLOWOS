...
export default function ConnectContactsPage() {
  ...
  const load = useCallback(async () => {
    if (!businessId) {
      setLoading(false);
      setError("Pick an active workspace first.");
      return;
    }
    setLoading(true);
    setError(null);
    const [statusRes, recentRes, teamRes] = await Promise.all([
      apiGet<ContactStatus>(`/connect/businesses/${businessId}/contacts/status`),
      apiGet<RecentContact[]>(`/connect/businesses/${businessId}/contacts/recent?limit=25`),
  ...
  const handleSync = async () => {
    ...
    setSyncing(true);
    setLastSummary(null);
    // Poll status while the sync runs so the UI shows progress.
    const poll = setInterval(async () => {
      const r = await apiGet<ContactStatus>(`/connect/businesses/${businessId}/contacts/status`);
  ...
  const handleSaveRules = async () => {
    ...
    const res = await apiPut(
      `/connect/businesses/${businessId}/contacts/mapping-rules`,
      {
        defaultTags: tags,
  ...
  return (
    <div className="space-y-5 max-w-5xl mx-auto">
      ...
      <PageHeader
        ...
        subtitle={
          status?.connected
            ? `Connected as ${status.account ?? "Google account"}.`
            : "Connect Google Contacts on the KeyFlow Connect page to sync your address book."
        }
        rightSlot={
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Users}
          title="Google Contacts isn’t connected"
          description="Connect Google Contacts from KeyFlow Connect, then come back to sync."
          ...
          <section className="rounded-2xl border border-border/50 bg-card/40 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[11px] text-muted-foreground">
                Applied to every newly imported Google contact.
              </span>
            </div>
          ...
          <section>
            ...
            {recent.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-border/40 p-6 text-center text-xs text-muted-foreground">
                No Google contacts have been imported yet. Run a sync to pull them in.
              </div>
            ) : (
