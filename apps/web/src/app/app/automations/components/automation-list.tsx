...
export function AutomationList({ businessId, templateToUse, onTemplateClear }: AutomationListProps) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [pbRes, wfRes] = await Promise.all([
        fetchPlaybooks(bid),
        fetchCrossModuleWorkflows(bid),
      ]);
      setPlaybooks(pbRes.data ?? []);
      setWorkflows(wfRes.data ?? []);
      if (pbRes.error) setError(pbRes.error);
      if (wfRes.error) setError(wfRes.error);
      setLoading(false);
    };
    void load();
  }, [businessId]);
  ...
  const unified: UnifiedAutomation[] = [
  ...
  const totalCount = playbooks.length + workflows.length;
  ...
  async function handleTogglePlaybook(pb: Playbook) {
    ...
    if (err) setError(err);
    else if (data) setPlaybooks((prev) => prev.map((p) => (p.id === pb.id ? data : p)));
  }
  ...
  async function handleToggleWorkflow(wf: CrossModuleWorkflow) {
    ...
    if (err) setError(err);
    else setWorkflows((prev) => prev.map((w) => (w.key === wf.key ? { ...w, enabled: !w.enabled } : w)));
    setUpdatingWf(null);
  }
  ...
  async function handleConfigUpdate(wf: CrossModuleWorkflow, newConfig: Record<string, string | number | boolean | null>) {
    ...
    if (err) setError(err);
    else setWorkflows((prev) => prev.map((w) => (w.key === wf.key ? { ...w, config: newConfig } : w)));
    setUpdatingWf(null);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="text-sm text-muted-foreground py-8 text-center">Loading automations...</div>
      ) : filtered.length === 0 ? (
        <div className="rounded-2xl border border-border/60 bg-card p-8 text-center">
          ...
          <p className="text-sm font-medium mb-1">
            {search.trim() ? "No matching automations" : "No automations yet"}
          </p>
