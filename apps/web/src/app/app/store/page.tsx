      ...
      <div {...swipeHandlers} className="touch-pan-y">
        <AnimatePresence mode="wait" custom={dirRef.current}>
          <motion.div key={activeTab} custom={dirRef.current} variants={SLIDE} initial="enter" animate="center" exit="exit" transition={{ type: "spring", bounce: 0.15, duration: 0.35 }}>
            ...
            {activeTab === "performance" && <PerformanceTab businessId={s.businessId} storeEnabled={s.storeEnabled} publicUrl={s.getPublicBookingUrl()} servicesCount={s.services.length} productsCount={s.commerceProducts.length} driftedCount={s.driftedItems.length} hasLogo={!!s.businessData?.logoUrl} hasHeroImage={!!(cfg.hero as any)?.imageUrl || !!(cfg.hero as any)?.coverImageUrl} hoursConfigured={Object.values(s.businessHours).some((h: any) => h?.enabled)} hasTestimonials={!!((cfg.socialProof as any)?.testimonials?.length)} hasSlug={!!s.businessData?.slug} analytics={s.overviewAnalytics} businessName={s.businessData?.name} onTabChange={handleTabChange} />}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
