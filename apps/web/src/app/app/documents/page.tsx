...
import {
  ...
  ChevronRight, Search, Filter, Building2, DollarSign,
  Palette, Settings, Users, Lock, Globe, TrendingUp,
  Truck, Award, Home, Package, Scale, Globe2,
  ShieldCheck, UserCheck, Sparkles, Clock, Eye,
} from "lucide-react";
...
export default function DocumentsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const catRes = await apiGet<DocumentCategory[]>(`/documents/categories?businessId=${bid}`);
      if (catRes.data) setCategories(catRes.data);

      const [instRes, healthRes] = await Promise.all([
        apiGet<DocumentInstance[]>(`/documents/businesses/${bid}/instances`),
        apiGet<HealthData>(`/documents/businesses/${bid}/health`),
      ]);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[hsl(var(--foreground))]">Business Documents</h1>
          <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
            Generate, manage, and track all your business documents from one place
          </p>
        </div>
      </div>
      ...
      {health && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <div className="p-3 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))]">
            <div className="text-2xl font-bold text-[hsl(var(--foreground))]">{health.healthScore}%</div>
            <div className="text-xs text-[hsl(var(--muted-foreground))]">Health Score</div>
          </div>
          <div className="p-3 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))]">
            <div className="text-2xl font-bold text-[hsl(var(--foreground))]">{health.total}</div>
            <div className="text-xs text-[hsl(var(--muted-foreground))]">Total Docs</div>
          </div>
          <div className="p-3 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))]">
            <div className="text-2xl font-bold text-[hsl(var(--kf-success))]">{health.current}</div>
            <div className="text-xs text-[hsl(var(--muted-foreground))]">Current</div>
          </div>
          <div className="p-3 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))]">
            <div className="text-2xl font-bold text-[hsl(var(--kf-warning))]">{health.stale}</div>
            <div className="text-xs text-[hsl(var(--muted-foreground))]">Stale</div>
          </div>
          <div className="p-3 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))]">
            <div className="text-2xl font-bold text-[hsl(var(--kf-error))]">{health.impacted}</div>
            <div className="text-xs text-[hsl(var(--muted-foreground))]">Impacted</div>
          </div>
          <div className="p-3 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))]">
            <div className="text-2xl font-bold text-[hsl(var(--kf-warning))]">{health.pendingReview}</div>
            <div className="text-xs text-[hsl(var(--muted-foreground))]">Reviews</div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select
          ...
          className="px-3 py-2 rounded-lg bg-[hsl(var(--card))] border border-[hsl(var(--border))] text-sm min-h-[44px] text-[hsl(var(--foreground))]"
        >
          ...
          {coreCategories.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider">Universal Core</h2>
                <span className="text-xs text-[hsl(var(--muted-foreground))]">Every business needs these</span>
              </div>
          ...
          {triggeredCategories.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider">Triggered Core</h2>
                <span className="text-xs text-[hsl(var(--muted-foreground))]">Required when conditions apply</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {advancedCategories.length > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider">Advanced</h2>
                <span className="text-xs text-[hsl(var(--muted-foreground))]">For scale, regulation, or expansion</span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-3">
          {instances.length === 0 ? (
            <div className="text-center py-16 bg-[hsl(var(--card))] rounded-xl border border-[hsl(var(--border))]">
              ...
              <h3 className="text-lg font-semibold text-[hsl(var(--foreground))]">No documents yet</h3>
              <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1 mb-4">
                Switch to the Document Catalog to generate your first document
              </p>
              <button
                type="button"
                onClick={() => setView("catalog")}
            ...
            instances.map((inst) => (
              <button
                ...
                className="w-full flex items-center gap-4 p-4 rounded-xl bg-[hsl(var(--card))] border border-[hsl(var(--border))] hover:border-[hsl(var(--kf-accent1))]/30 transition-colors text-left min-h-[44px]"
              >
                <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-[hsl(var(--kf-accent1))]/10 flex items-center justify-center">
                  <FileText className="w-5 h-5 text-[hsl(var(--kf-accent1))]" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-[hsl(var(--foreground))] truncate">{inst.title}</span>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg bg-[hsl(var(--background))] border border-[hsl(var(--border))] text-sm text-[hsl(var(--foreground))] resize-none"
                />
              </div>
              <div>
