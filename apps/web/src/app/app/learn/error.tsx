...
export default function LearnError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("Learn module error:", error);
  }, [error]);
  ...
  return (
    <div className="flex items-center justify-center min-h-[400px] p-6">
      <div className="max-w-md w-full kf-card border border-border/60 p-8 text-center space-y-5">
        ...
        <div>
          <h2 className="text-lg font-semibold text-foreground mb-1">MasterClass Error</h2>
          <p className="text-sm text-muted-foreground">
            Something went wrong loading the learning module. Your progress is safe.
          </p>
        </div>
