...
export function WhatsAppManageDrawer({ businessId }: { businessId: string }) {
  ...
  if (!status?.connected) {
    return (
      <div className="flex flex-col h-full p-6 text-center bg-white">
        <MessageCircle className="w-10 h-10 mx-auto text-emerald-500 mb-3" />
        <h3 className="text-base font-semibold mb-1">WhatsApp not connected</h3>
        <p className="text-sm text-neutral-600">
          Link a WhatsApp Business number in the Key Connect hub to send and receive messages.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-xs text-neutral-500">{status.destination?.displayName ?? "Connected"}</div>
          </div>
        </div>
        <Button
          size="sm"
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Conversations list */}
        <div className="w-64 border-r border-neutral-200 bg-white overflow-y-auto">
              ...
              {conversations.map((c) => (
                <li key={c.id}>
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 hover:bg-neutral-50 ${activeId === c.id ? "bg-emerald-50" : ""}`}
                  >
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="font-medium text-sm truncate">{c.displayName}</span>
                      <span className="text-[10px] text-neutral-400">{formatTime(c.lastMessageAt)}</span>
                    </div>
          ...
          {!conversation ? (
            ...
            <>
              <div className="px-4 py-3 border-b border-neutral-200 flex items-center justify-between">
                <div>
                  <div className="font-medium text-sm">{conversation.displayName}</div>
                  <div className="text-xs text-neutral-500">+{conversation.phoneNumber}</div>
                </div>
                <Badge className="text-xs">{conversation.withinWindow ? "24h window open" : "Out of window"}</Badge>
              </div>
      ...
      {showNew && (
        <div className="absolute inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-5 w-full max-w-sm space-y-3 m-4">
            <h3 className="text-base font-semibold">New WhatsApp conversation</h3>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <Button variant="outline" size="sm" onClick={() => setShowNew(false)} disabled={sending}>
                Cancel
              </Button>
              <Button size="sm" onClick={handleNewConversation} disabled={sending}>
