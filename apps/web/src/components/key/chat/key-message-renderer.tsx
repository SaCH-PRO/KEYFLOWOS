...
import { KeyInlinePreview } from "./key-inline-preview";
    ...
    <div className={cn("flex flex-col gap-2 w-full", isUser ? "items-end" : "items-start")}>
      ...
      {message.content ? (
        isUser ? (
          ...
          <div className="prose prose-sm dark:prose-invert max-w-none text-sm leading-relaxed w-full">
            <ReactMarkdown
      ...
      {message.preview && (
        <div className="w-full">
          <KeyInlinePreview preview={message.preview} onAction={(value) => onConfirm?.("preview-action", true, "preview", { value })} />
        </div>
      )}
