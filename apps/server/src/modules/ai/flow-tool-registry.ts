...
export const FLOW_TOOLS: FlowTool[] = [
  ...
  {
    name: 'contract_list',
    description: 'List contracts with optional filters (status, search, expiring soon).',
    family: 'read',
    riskLevel: 'low',
    riskTier: 1 as RiskTier,
    ...
    parameters: {
      type: 'object',
      properties: {
        status: { type: 'string', description: 'Filter by status (DRAFT, ACTIVE, EXPIRED, TERMINATED, etc.)' },
        search: { type: 'string', description: 'Search title/counterparty' },
        expiringWithinDays: { type: 'number', description: 'Only contracts expiring within N days' },
        limit: { type: 'number', description: 'Max results (default 25)' },
      },
      required: [],
    },
    outputSchema: { type: 'object', description: 'Contracts list', fields: { contracts: { type: 'array', description: 'Contract rows' }, total: { type: 'number', description: 'Total count' } } },
  },
  {
    name: 'contract_get',
    description: 'Get full details of a contract including terms, parties, dates, and extracted key clauses.',
    family: 'read',
    riskLevel: 'low',
    riskTier: 1 as RiskTier,
    manualEquivalentRoute: '/app/contracts',
    ...
    outputSchema: { type: 'object', description: 'Contract detail', fields: { contract: { type: 'object', description: 'Full contract record' } } },
  },
  {
    name: 'contract_create',
    description: 'Create a new contract record (draft or active).',
    family: 'organize',
    riskLevel: 'medium',
    riskTier: 2 as RiskTier,
    manualEquivalentRoute: '/app/contracts',
    ...
    parameters: {
      ...
      properties: {
        ...
        contractType: { type: 'string', description: 'Type: SERVICE, NDA, EMPLOYMENT, VENDOR, LEASE, PARTNERSHIP, OTHER' },
        status: { type: 'string', description: 'Initial status (default DRAFT)' },
        counterpartyName: { type: 'string', description: 'Other party name' },
        startDate: { type: 'string', description: 'Start date (ISO)' },
        endDate: { type: 'string', description: 'End/expiry date (ISO)' },
        notes: { type: 'string', description: 'Optional notes' },
      },
      required: ['title'],
    },
    outputSchema: { type: 'object', description: 'Created contract', fields: { id: { type: 'string', description: 'Contract ID' }, title: { type: 'string', description: 'Contract title' } } },
  },
  {
    name: 'contract_update',
    description: 'Update a contract (status, dates, counterparty, notes).',
    family: 'organize',
    riskLevel: 'medium',
    riskTier: 2 as RiskTier,
    manualEquivalentRoute: '/app/contracts',
    ...
    parameters: {
      ...
      properties: {
        ...
        endDate: { type: 'string', description: 'New end/expiry date (ISO)' },
        counterpartyName: { type: 'string', description: 'Counterparty name' },
        notes: { type: 'string', description: 'Notes' },
      },
      required: ['contractId'],
    },
    outputSchema: { type: 'object', description: 'Updated contract', fields: { id: { type: 'string', description: 'Contract ID' }, status: { type: 'string', description: 'New status' } } },
  },
  {
  ...
  {
    name: 'contract_get_stats',
    description: 'Contract portfolio stats: totals by status and expiry risk.',
    family: 'read',
    riskLevel: 'low',
    riskTier: 1 as RiskTier,
    manualEquivalentRoute: '/app/contracts',
    parameters: { type: 'object', properties: {}, required: [] },
    outputSchema: { type: 'object', description: 'Contract stats', fields: { total: { type: 'number', description: 'Total contracts' }, byStatus: { type: 'object', description: 'Counts by status' } } },
  },
  {
    name: 'contract_acknowledge_alert',
    description: 'Acknowledge a contract renewal/expiry alert.',
    family: 'organize',
    riskLevel: 'low',
    riskTier: 2 as RiskTier,
    manualEquivalentRoute: '/app/contracts',
    ...
    outputSchema: { type: 'object', description: 'Acknowledged alert', fields: { id: { type: 'string', description: 'Alert ID' } } },
  },
  {
