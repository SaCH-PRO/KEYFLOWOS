...
export const FLOW_TOOLS: FlowTool[] = [
  ...
  },
  {
    ...
    riskTier: 2 as RiskTier,
    manualEquivalentRoute: '/app/bookings',
    changedEntities: ['bookingWaitlistEntry'],
    parameters: {
      type: 'object',
      properties: {
        contactId: { type: 'string', description: 'Contact ID to add to the waitlist' },
        serviceId: { type: 'string', description: 'Service ID the contact wants to book' },
        preferredStaffId: { type: 'string', description: 'Optional preferred staff member ID' },
        preferredDateFrom: { type: 'string', description: 'Optional earliest preferred date (ISO)' },
        preferredDateTo: { type: 'string', description: 'Optional latest preferred date (ISO)' },
        preferredTimeOfDay: { type: 'string', description: 'Optional time preference: morning, afternoon, or evening' },
        notes: { type: 'string', description: 'Optional notes about the waitlist request' },
      },
      required: ['contactId', 'serviceId'],
    },
    outputSchema: { type: 'object', description: 'Created waitlist entry', fields: { id: { type: 'string', description: 'Waitlist entry ID' }, status: { type: 'string', description: 'Waitlist entry status' } } },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        status: { type: 'string', description: 'Optional status filter: WAITING, OFFERED, CONVERTED, EXPIRED, CANCELLED' },
        serviceId: { type: 'string', description: 'Optional service ID filter' },
        contactId: { type: 'string', description: 'Optional contact ID filter' },
      },
      required: [],
    },
    outputSchema: { type: 'object', description: 'Waitlist entries', fields: { items: { type: 'array', description: 'Waitlist entry records' }, total: { type: 'number', description: 'Total matching entries' } } },
  },
  {
    ...
    riskTier: 2 as RiskTier,
    manualEquivalentRoute: '/app/bookings',
    changedEntities: ['booking', 'bookingWaitlistEntry'],
    parameters: {
      type: 'object',
      properties: {
        entryId: { type: 'string', description: 'Waitlist entry ID' },
        startTime: { type: 'string', description: 'Slot start time in ISO format' },
        endTime: { type: 'string', description: 'Slot end time in ISO format' },
        serviceId: { type: 'string', description: 'Service ID for the slot' },
        staffId: { type: 'string', description: 'Optional staff ID for the slot' },
      },
      required: ['entryId', 'startTime', 'endTime', 'serviceId'],
    },
    outputSchema: { type: 'object', description: 'Offer result', fields: { entryId: { type: 'string', description: 'Waitlist entry ID' }, bookingId: { type: 'string', description: 'Placeholder booking ID' } } },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        entryId: { type: 'string', description: 'Waitlist entry ID to cancel' },
      },
      required: ['entryId'],
    },
    outputSchema: { type: 'object', description: 'Cancellation result', fields: { success: { type: 'boolean', description: 'Whether cancellation succeeded' } } },
