...
export class BusinessGraphService {
  ...
  private async assembleSnapshot(businessId: string): Promise<BusinessGraphSnapshot> {
    ...
    const [
      ...
      db.expenseBudget.aggregate({ where: { businessId, deletedAt: null }, _sum: { amount: true } }),
      db.project.count({ where: { businessId, deletedAt: null, status: { in: ['ACTIVE', 'IN_PROGRESS'] } } }),
      db.projectTask.count({ where: { businessId, deletedAt: null, status: { not: 'COMPLETED' }, dueDate: { lt: now } } }),
      db.projectTask.count({ where: { businessId, deletedAt: null, status: 'COMPLETED' } }),
