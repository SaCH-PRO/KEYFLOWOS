...
import { ModuleName, ModuleCapability } from './key-cortex-connector.types';

/**
 * Central registry for every KeyFlowOS module capability exposed to KEY Cortex.
 *
 * v1 -- Extracted from KeyCortexConnectorService as part of Phase 0.7 god-service
 * decomposition.  Keeps the canonical capability data structure intact while
 * removing registry responsibility from the connector.
 */
@Injectable()
export class KeyCortexCapabilityRegistryService {
  private readonly logger = new Logger(KeyCortexCapabilityRegistryService.name);
