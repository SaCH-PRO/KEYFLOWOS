...
export class KeyCortexMetacognitionService {
  ...
  async explainConfidence(statementId: string): Promise<string> {
    ...
    const decisionEvent = events.find(
      (e) => e.eventType === BusinessEventType.DECISION_MADE,
    );
    if (!decisionEvent) {
  ...
  private async inferOutcome(
    ...
    const successSignals = followUps.filter(
      (e) =>
        e.eventType === BusinessEventType.RECOMMENDATION_ACTED ||
        (e.eventType === BusinessEventType.ACTION_EXECUTED &&
          (e.metadata as Record<string, unknown>).result === 'success') ||
        e.eventType === BusinessEventType.GENOME_EVOLUTION,
    ).length;
      ...
      .filter((e) =>
        keywords.some(
          (k) =>
            ...
            (((e.metadata as Record<string, unknown>)?.proof as Record<string, unknown>)?.description as string)
              ?.toLowerCase()
              .includes(k),
        ),
      )
  ...
  private async identifyDataGaps(businessId: string): Promise<string[]> {
    ...
    // Business has no `integrations` / `crmConnections` relations.
    // IntegrationConnection is its own model keyed by businessId, and contact
    // records stand in for CRM presence.
    const connections = await this.prisma.client.integrationConnection.findMany({
      where: { businessId },
      select: { providerKey: true },
    });
    const integrationTypes = connections.map((c: { providerKey: string }) => c.providerKey);
    const crmContactCount = await this.prisma.client.contact.count({
      where: { businessId },
    });
  ...
  private async upsertCapabilityRecord(
    ...
    await this.prisma.client.businessEvent.create({
      // BusinessEvent's real columns: eventType (not type), subjectType/subjectId
      // and actorType/actorId are required, and extensible context is `metadata`
      // (there is no `module` or `payload` column).
      data: {
        businessId,
        eventType: BusinessEventType.ACTION_EXECUTED,
        subjectType: 'capability',
        subjectId: capability,
        actorType: 'ai',
        actorId: 'metacognition',
        source: 'worker',
        action: 'capability_tracked',
        metadata: {
          capability,
