  ...
  private async collectTextCorpus(businessId: string, since: Date): Promise<Array<{ source: string; text: string; timestamp: Date }>> {
    ...
    for (const t of tickets ?? []) {
      corpus.push({
        ...
        text: `${(t as { title: string }).title} ${(t as { description?: string }).description ?? ''}`,
    ...
    for (const e of events) {
      if ((e as { description?: string }).description) {
        corpus.push({
          ...
          text: (e as { action: string }).action,
...
[{"theme": "...", "sources": ["..."], "severity": "low|medium|high", "confidence": 0.0-1.0}]`;
    ...
    try {
      const emailData = await this.prisma.client.emailCampaign
        ?.findMany({
          ...
          select: { id: true }, // Campaign has no openRate/clickRate/conversionRate/name columns
        })
        .catch(() => []);
  ...
  private async findSimilarComplaintClusters(
    ...
    for (const ticket of (tickets ?? []) as Array<{
      ...
      for (const cluster of clusters) {
        ...
        if (jaccard > 0.3) {
          if (ticket.contactId && !cluster.customerIds.includes(ticket.contactId)) {
            cluster.customerIds.push(ticket.contactId);
          }
      ...
      if (!merged) {
        clusters.push({
          ...
          customerIds: ticket.contactId ? [ticket.contactId] : [],
  ...
  private async buildChurnPatternLibrary(businessId: string): Promise<void> {
    ...
    try {
      ...
      for (const customer of churnedCustomers as Array<{
        id: string;
        ...
        case 'contract_downgrade': {
          ...
          const planChanges = await this.prisma.client.subscription
            ?.findMany({
              ...
              select: { id: true }, // Subscription has no seatCount column
