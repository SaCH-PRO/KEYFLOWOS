...
export class KeyCortexCreativityService {
  ...
  async brainstorm(
    ...
    for (const technique of techniquesToUse) {
      try {
        const techniqueIdeas = await this.runTechnique(technique, prompt, businessId, businessContext as unknown as Record<string, unknown>, genomeInsights, requestedCount);
  ...
  async generateNovelStrategy(businessId: string, goal: string): Promise<CreativeIdea[]> {
    ...
    const contextSummary = this.buildContextSummary(businessContext as unknown as Record<string, unknown>, genomeInsights, recentInsights);
...
[{
  ...
  "tags": ["..."]
    ...
    try {
      ...
      ideas = parsed.map((p) =>
          this.enrichIdea(
            { ...p, technique: p.technique as CreativityTechnique },
            businessId,
            sessionId,
          ),
        );
      ...
      ideas = await this.fallbackStrategyGeneration(businessId, goal, businessContext as unknown as Record<string, unknown>);
    }
...
[{
  ...
  "inspiration": "What ${company.name} would do: ${company.principles.join(', ')}"
      ...
      try {
        ...
        ideas.push(
          ...this.fallbackAnalogyIdea(businessId, company.name, company.principles),
        );
      }
    }
...
[{
  ...
  private async getGenomeInsights(businessId: string): Promise<string[]> {
    try {
      ...
      return Object.entries(genome as unknown as Record<string, unknown>).map(([k, v]) => `${k}: ${JSON.stringify(v).slice(0, 200)}`);
    } catch {
      return [];
    }
  }
