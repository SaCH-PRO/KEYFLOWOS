...
- **Community Visible AI Matchmaking:** When a user posts a `QUESTION`/`OPPORTUNITY`/`HELP`/`NEED`, `CommunityService.notifyMatchedProvidersForPost` finds and notifies top candidate providers.
...
- **Launch & Maintenance Hardening:** Both apps validate environment with Zod at startup. API exposes `GET /healthz` and `GET /readyz`. Web exposes `GET /api/healthz`. Production deploy uses `scripts/start-prod.sh`. `scripts/verify-up.sh` for smoke tests. `scripts/post-merge.sh` checks for Prisma schema drift.
- **Uptime monitoring:** `scripts/uptime-monitor.sh` (root `pnpm monitor:uptime`) is a dependency-free shell poller that hits `/api/healthz` and `/readyz`, asserts JSON keywords, persists counters, and pages on the 2nd consecutive failure. Supported by `.github/workflows/uptime-monitor.yml`.
- **Dev Auth Bypass:** Development-only feature to auto-authenticate as a seeded `keyflowdev` `SUPER_ADMIN` profile. Hard-fails in production.
