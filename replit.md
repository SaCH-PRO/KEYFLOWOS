...
- **CRM Insights Tab (Production-Grade):** Refactored `insights-tab.tsx` with 8 sections: Pipeline Funnel (horizontal bar chart with conversion % between stages), Conversion Metrics (4-card grid: overall conversion, new this week, conversions this week, lost), Alert Cards (priority-sorted: ready-to-advance > overdue invoices > going cold > expiring quotes, with ARIA labels), Revenue Forecast (stacked bar with TTD formatting via `formatTTD` helper), Contact Growth (period-aware bar chart with bucket labels), Data Completeness (circular SVG progress ring, weighted field scoring across 11 fields, per-field breakdown), Task & Revenue Health (overdue tasks, unpaid invoices, total revenue from meta), Lead Score Distribution (Hot/Warm/Cool/Cold buckets from `leadScore`). All sub-components wrapped in `React.memo`. Per-section empty states (no blank gaps). Period dropdown with `role="listbox"`, `aria-expanded`, `aria-selected`. TTD locale formatting. `useCallback` for all handlers.
