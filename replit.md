...
- **Commerce Module Overhaul (v2):** Re-architected with `CommerceService` (core CRUD), `CommerceStatsService` (server-side KPIs with 60s TTL cache), and `CommerceAiService` (revenue analysis, cash flow forecast, NL command interpreter). Frontend features `CommerceAiSearchBar` and `useCommerceAiHub` with various AI tools. Unified currency system (`apps/web/src/lib/currency.ts`) with 24 supported currencies, `formatCurrency(amount, currency)` utility, and `SUPPORTED_CURRENCIES` list. Shared `LineItemsEditor` component eliminates quote/invoice line-items duplication. Commerce hooks decomposed: `useProducts`, `useCommerceIntegrations` composed by thin `useCommerce` orchestrator. Invoice delete uses secure `apiDelete`. Typed `QuoteFormState`/`InvoiceFormState`. Recurring invoice scheduler (`onModuleInit` hourly). Expired quote badges with disabled accept. Per-action loading spinners. Mobile-visible product card actions. Sort dropdown with arrow key navigation. **Product Import System:** Multi-mode import via `ProductImportModal` — CSV file upload (with header synonym mapping and downloadable template) and AI Vision image scan (GPT-4o extracts products from photos of menus, catalogs, flyers). Backend `CommerceVisionService` handles AI extraction and CSV parsing. Preview step with inline editing, selection toggles, and batch confirm. "Save & Add Another" rapid entry in product form modal. Dashboard tab removed; Gmail status and payment gateway configuration moved to Settings. **Unified Billing Panel:** Commerce reduced from 5 tabs to 2 (Products + Billing). `BillingPanel` (`billing/billing-panel.tsx`) hosts glass segment nav for Quotes | Invoices | Schedules with count badges, KPI chips (Revenue, Owed, Overdue, Conversion), AnimatePresence slide transitions, and `defaultSegment` prop. Sub-panels (`QuotesPanel`, `InvoicesPanel`, `RecurringPanel`) are self-contained — each imports its own form hook (`useQuoteForm`/`useInvoiceForm`) and manages builder state internally. External triggers via `triggerNew` counter props. Event-driven pluggability via `ModuleEventBus` emissions: `billing:quote_created`, `billing:quote_accepted`, `billing:quote_converted`, `billing:invoice_created`, `billing:invoice_sent`, `billing:invoice_paid`, `billing:schedule_created`, `billing:schedule_toggled`, `billing:segment_changed`. `CommerceGuide` sections route to `billing` tab.
