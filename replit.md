...
- **Commerce Module:** Manages Quotes, Invoices, Schedules, and Insights. Features comprehensive CRUD, KPI tracking with caching, AI-powered revenue analysis, cash flow forecasting, NL interpretation, and a unified currency system. Supports recurring invoices, partial payments, and customizable invoice/quote templates via a `TemplateStudio`. Commerce Phase 2 introduced a 3-pane workbench layout (saved-views rail, center record list, right detail/inspector pane), right-sliding `SideSheet` builders replacing modals, context-driven smart CTAs per row (status-based primary action with overflow for secondary/destructive actions), 4 standardized card types (`ActionCard`, `MetricCard`, `RecordRowCard`, `InsightCard`), and decluttering rules (max 4 quick filters visible, max 1 primary CTA per zone, destructive actions in overflow only).
