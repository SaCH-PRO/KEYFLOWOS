...
- **Real-Time Cockpit Dashboard:** Live business intelligence with a FlowService backend aggregating CRM, invoices, bookings, and quotes. Features momentum calculation, bottleneck detection, AI suggestions, Today's Priorities (overdue invoices, unconfirmed bookings, stale leads, draft posts with urgency-based sorting), and Revenue Insights (avg client spend, top service, lead conversion rate, retention rate, revenue growth, collection rate). WhatsApp deep links throughout for one-tap messaging.
- **Online Store & Public Booking Page:** Modular component architecture. **Store Admin** expanded to 5-tab layout (Overview, Customize, Products, Hours, Settings) with 8+ components: StoreHeader (publish toggle, copy link, open store), StoreSettings (slug editor with live validation, WhatsApp share), HoursEditor (business hours with day toggles), CatalogManager (product toggles with search/filter/select-all), StorefrontPreview (visual preview with item cards), AppearanceCustomizer (hero config, theme presets, card layout, SEO settings), MerchandisingPanel (featured items, badges Popular/New/Best Seller/Limited, promotions banner with color/expiry), StoreAnalyticsDashboard (KPIs, booking/revenue/catalog metrics, storefront event breakdown). Storefront config stored in Business.metaData.storefront JSON (no schema migration needed). **Public Booking** split into 7 components: BusinessHero (logo, tagline, contact info, cover image), CatalogGrid (filterable/searchable/sortable with badge chips, featured item sorting, click-to-detail), ItemDetailModal (full item details, trust badges, WhatsApp inquiry, related items), CartDrawer (slide-out cart with quantity controls), CheckoutFlow (4-step: Review → Schedule → Details → Confirm). Promotions banner, social proof testimonials with star ratings, guarantee text display. Analytics event tracking (page_view, item_view, add_to_cart, checkout_start, checkout_complete). Server-side OG metadata via generateMetadata for social sharing previews. WhatsApp share integration. Cart persisted in localStorage. Staff assignment optional for public bookings. SiteService backend with storefront config CRUD, public storefront endpoint, analytics event tracking (bounded to last 1000 events in metaData).
...
- **Social:** Posts, scheduling, multi-platform channel integration (Facebook, Instagram, LinkedIn, Twitter, TikTok). OAuth + manual token connect/disconnect. SocialConnectionsService for CRUD, SocialPublishingService orchestrator with platform-specific publishers (FacebookPublisher, InstagramPublisher, LinkedInPublisher using modern Posts API v202401, TwitterPublisher, TikTokPublisher using Content Posting API). Channel selector in PostComposer for targeted publishing. SocialAnalyticsService fetches real engagement data (likes, comments, shares, reach, impressions) from all 5 platform APIs. Analytics dashboard with platform breakdown, engagement rates, and per-post metrics. TikTok OAuth authorization flow (start + callback with token exchange).
...
- **Cockpit:** Flow Graph, Flow Feed wired to real activity data (not mock), live momentum bar calculated from actual business signals, universal search via Cmd+K command palette searching all modules. Today's Priorities with urgency-based sorting and WhatsApp one-tap messaging. Revenue Insights with business intelligence metrics. booking.completed event auto-generates draft invoices for completed bookings.
