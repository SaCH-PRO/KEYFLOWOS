...
- **Profile Module:** Modular accordion-based UI (personal-info-section, professional-profile-section, security-section, document-health-section) with profile completeness ring, CSS variable theming, and unsaved-changes protection. Business Guidance Engine dashboard with AI health scores and roadmap. Document health section shows health score ring, document counts, and impact alerts.
- **Business Documentation Engine (`/app/documents`):** AI-powered document generation, versioning, and health tracking across 18 categories (6 Universal Core + 10 Triggered Core + 2 Advanced). Features: document generator with AI drafting via BusinessContextService, section-level editing with risk-tiered edit modes (GREEN/YELLOW/RED), AI tweak bar, version history, audit logs, review task resolution, health dashboard with impact detection. Profile changes automatically detect and flag impacted documents via ImpactRule system (81 seeded rules). Models: DocumentCategory, DocumentType, DocumentInstance, DocumentVersion, DocumentSection, ReviewTask, DocumentChangeLog, ImpactRule, OrgStandard, BusinessProfileVersion.
