...
- **Store Module:** Redesigned with Storefront, Products & Hours, and Performance tabs. Features a consolidated setup surface for the storefront, detailed product and hour management, comprehensive performance analytics, SEO settings panel (meta title/description with Google search preview), and Share Store button with URL copy. Commerce Order Engine with `StoreOrderService` for full order lifecycle (cart validation → promo code → tax/shipping → order creation → payment → confirmation), `PromoCodeService` (PERCENT/FIXED/FREE_SHIPPING with validity dates, usage limits, min order), guest checkout via public endpoints, payment gateway integration (WiPay, PayPal, CASH/MANUAL), seller order management with status tracking and refunds, and webhook events for order lifecycle changes. Enhanced with section-based layout builder (drag-to-reorder, toggle sections), FAQ manager (CRUD with reorder), policy editor (refund/privacy/delivery/terms with enable toggles), font pairing system (10 curated Google Font combinations with live preview), store settings panel (delivery options, store status, currency, contact options), and upgraded storefront preview rendering all configured sections with badges (Popular, New, Best Seller, Limited, Sale) and font pairings. Featured items capped at 8.
...
- **Business Guidance Engine:** Schema and backend foundation for structured business profiles with diagnostic scoring, financial analysis, and recommendations. Prisma models: `BusinessGuidanceProfile` (master), 9 sub-profiles (Founder, Offer, Customer, Revenue, Finance, Operations, Compliance, Growth, Goals), plus `AssessmentResult`, `GuidanceRecommendation`, `RoadmapItem`, `ProgressSnapshot`. NestJS module at `apps/server/src/modules/business-guidance/` with 7 services (Profile, Scoring, Financial, Recommendation, Roadmap, AiFeedback, Assessment). Full CRUD with draft save/resume, per-step DTO validation, and 11 REST endpoints under `/business-guidance/:businessId/`.
