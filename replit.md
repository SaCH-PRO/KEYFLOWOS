...
- **Commerce Module:** Re-architected with `CommerceService` (CRUD), `CommerceStatsService` (KPIs with caching), and `CommerceAiService` (revenue analysis, cash flow forecast, NL interpreter). Includes a unified currency system, recurring invoice scheduler, product import system (CSV & AI Vision), and a unified billing panel. Features a responsive timeline view for quotes and invoices, and a pluggable architecture using `CommerceSlots`.
- **CRM Module:** Modular CRM with Contact Lists/Groups, Duplicate Detection, Pipeline, Database, Insights, Engage tabs, and AI copilot. Includes bulk edit, inline editing, import field mapping, virtual scrolling, and a pluggable notes and tasks system.
