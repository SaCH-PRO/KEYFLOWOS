...
- **Autopilot Governed Delegation Engine:** 5 pre-built delegation loop templates (Payment Recovery, Lead Reactivation, Post-Purchase Follow-up, Booking Prep, Weekly Hygiene) with governance-aware execution via GovernanceService (respects tier-based approval/confirm/auto-execute policies), learning loop integration via AiMemoryService, 5-minute scheduler, and an Autopilot dashboard tab on the Flows/Automations page. Models: `DelegationLoop` + `DelegationLoopRun`. API: `/autopilot/businesses/:businessId/loops[/:loopId[/run|/runs]]`.
