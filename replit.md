...
- **CRM Insights Tab (Production-Grade, Bento Layout):** Redesigned `insights-tab.tsx` using Bento Grid layout (Apple/Vercel-inspired). **Row 1:** HeroStats — 4 prominent stat cards (Total Contacts, Conversion Rate, Pipeline Value, New This Week) with gradient accent circles, trend indicators, and `ArrowUpRight`/`ArrowDownRight` micro-trends. **Row 2:** Asymmetric 3:2 grid — Pipeline Funnel (gradient bars, conversion % arrows) + Action Items (priority-sorted alerts with hover arrow reveal, "All clear" empty state). **Row 3:** Revenue Forecast (gradient stacked bar, line-item breakdown) + Contact Growth (bar chart with always-visible counts and `aria-label`). **Row 4:** 3-column grid — Data Completeness (SVG ring with proper `circumference` math, field breakdown bars), Lead Scores + Health (stacked card with fallback empty state), Sources + Segments (inline horizontal bars). All sections use uppercase `tracking-wider` headers, `rounded-2xl` cards with `border-border/50`, gradient fills (`bg-gradient-to-r`), and `backdrop-blur-xl` period dropdown. Sub-components wrapped in `React.memo`, `useCallback` for handlers, `useMemo` for derived data. TTD formatting via `formatTTD` helper. ARIA roles on all interactive elements.
