...
- **Profile Module:** Two-tab layout (Profile / Documents) with URL query param sync (`?tab=documents`). Profile tab: personal-info-section, professional-profile-section, security-section with profile completeness ring, **Business Builder** (interactive AI-powered business planning system with 7-step intake wizard → generates complete Business Model Canvas, execution roadmap, action plan, financial outlook, and risk assessment; results persist server-side in `BusinessPlan` model with versioning — supersedes localStorage; "Generate Documents" CTA deep-links to Documents tab with `?generate=<slug>` to auto-open the document generator for specific document types using real catalog slugs; section-level inline editing on canvas blocks with auto-save to server; action items have "Create task" buttons that create real `ProjectTask`s in the Projects module; action items show module links (bookings/commerce/marketing/etc) for direct navigation; 4-phase progress tracker with real API checks for bookings, campaigns, recurring invoices, reports, and AI usage; frontend + backend AI response validation with graceful fallbacks for missing sections), CSS variable theming, and unsaved-changes protection. Documents tab: full document catalog, my documents list, health dashboard, AI generator modal with `?generate=<slug>` deep-link support, and Google Drive browser with inline doc viewing/editing (embedded via `documents-tab.tsx` + `google-drive-browser.tsx`). The standalone `/app/documents` route redirects to `/app/profile?tab=documents`; document detail pages remain at `/app/documents/[instanceId]`.
...
- **Document Actions (Detail Page):** Four action buttons on every generated document: **Save to Drive** (creates Google Doc in user's Drive), **Email** (sends formatted document to user's inbox via Gmail/transactional email), **Download** (saves as .txt file to device), **Print** (opens formatted HTML print view). All actions available from `apps/web/src/app/app/documents/[instanceId]/page.tsx`.
