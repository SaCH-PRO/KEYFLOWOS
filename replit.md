...
- **Workspaces:** Dedicated workspaces for Revenue, Clients, Calendar, Content, Autopilot, Profile & Intelligence, Projects, and Expenses. The Clients surface (`/app/crm/clients`) is search-first: a centered search bar surfaces top-5 matching contacts as the user types (debounced, keyboard-navigable), and selecting a contact opens the existing contact detail full-screen with a back-to-search action. The legacy pipeline view remains at `/app/crm/pipeline`.
