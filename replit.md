...
- **CRM AI Command Executor:** Natural language command system in the AI search bar (`ai-search-bar.tsx`) that interprets user commands and executes CRM actions via backend `interpretCommand()` in `CrmAiService`. Supports 30+ action types including sequence enrollment, list management, bulk operations, duplicate detection, data cleanup, quote/invoice creation, and revenue/reengagement scans. Server-side execution via `POST /ai-execute` for 11 action types (enroll_sequence, unenroll_sequence, create_list, add_to_list, remove_from_list, bulk_status_change, add_note, add_task, change_status, toggle_favorite, log_communication).
- **CRM AI Intelligence Suite:** AI-powered features integrated into the Contacts/CRM module via `CrmAiService` + `AiUsageService.callAi()`: AI Contact Summarizer, Smart Lead Scoring, AI Note Intelligence, Predictive Churn Detection, Natural Language CRM Search, Data Quality Scan, Duplicate Finder, Re-engagement Planner, Revenue Opportunity Scanner, and AI Follow-up Drafter. Eleven AI Command Hub tools with dedicated result renderers.
- **AI Command Hub System:** Unified AI access point per module—a single floating button that expands into a full-featured panel with two modes: **Tools** (11 categorized AI capabilities with execute/result flow) and **Insights** (proactive AI suggestions).
...
- **Webhook Dispatcher System:** `WebhookDispatcherService` listens to all `EventEmitter2` events and fires registered webhook URLs with HMAC-SHA256 signatures for integration with platforms like Zapier. Retry with exponential backoff (3 attempts, 1s/4s/16s delays) and structured delivery logging.
...
- **CRM Performance:** In-memory 60s TTL cache on `crm-stats.service.ts` (getContactStats, segmentSummary, flowHighlights) and `crm-flow.service.ts` (getFlowIntelligence). Cache invalidation on all write operations. Cache hit/miss metrics tracked via `getCacheMetrics()`. Revenue forecast queries bounded to 90 days. Next actions query limited to 50 contacts.
- **Observability System:** Request correlation ID middleware (`CorrelationIdMiddleware`) generates UUID per request, reads/sets `X-Request-ID` header. Global `LoggingInterceptor` logs method, path, status, duration, and correlation ID for every request; warns on slow requests (>1000ms). CRM health check endpoint (`GET /crm/health`) verifies DB connectivity, cache metrics, and uptime without auth. Error responses include `correlationId` for request tracing.
