...
- **System Diagnostics & Health Center:** Real-time on-demand diagnostics dashboard at `/admin/system` (renamed from System Mind). `DiagnosticsService` (`apps/server/src/modules/diagnostics/`) runs checks across 5 categories: Infrastructure (PostgreSQL/Prisma, Supabase, Object Storage, Server Uptime/Memory), Modules (CRM, Commerce, Bookings, Identity, AI, Automation, Payments, Notifications, Reports, Marketing), Integrations (OpenAI API, Google OAuth, PayPal, WiPay, Facebook, Instagram, LinkedIn, Twitter, TikTok), Cross-Module Flows (Contact→Timeline, Invoice→CRM Contact, Booking→Service), and Environment Variables (required and optional vars audited by presence and format). Each check returns status (pass/warn/fail), latency, timestamp, and optional error detail. REST endpoints at `/api/diagnostics/*` (super-admin protected): `POST /run` (full sweep), `GET /infrastructure`, `GET /modules`, `GET /integrations`, `GET /cross-module`, `GET /env-vars`. tRPC router `diagnostics.*` also wired in `@keyflow/api`. Frontend shows health score ring, per-category expandable cards, color-coded status badges, and per-check latency + error drilldown. "Run Full Diagnostic" button triggers all checks with animated loading state.
