...
- **Store Module:** Redesigned with Storefront, Products & Hours, and Performance tabs. Features a consolidated setup surface for the storefront, detailed product and hour management, comprehensive performance analytics, SEO settings panel (meta title/description with Google search preview), and Share Store button with URL copy. Commerce Order Engine with `StoreOrderService` for full order lifecycle (cart validation → promo code → tax/shipping → order creation → payment → confirmation), `PromoCodeService` (PERCENT/FIXED/FREE_SHIPPING with validity dates, usage limits, min order), guest checkout via public endpoints, payment gateway integration (WiPay, PayPal, CASH/MANUAL), seller order management with status tracking and refunds, and webhook events for order lifecycle changes.
