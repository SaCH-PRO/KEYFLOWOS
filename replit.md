...
- **Marketing Module:** Redesigned with 4 tabs: Create & Schedule | Calendar | Audiences & Forms | Performance. Create tab merges Email Campaigns + Social Posts side-by-side. Calendar tab shows unified content calendar with campaigns and social posts in month/week views with type filters. `/app/social` redirects to `/app/marketing?tab=create`. Legacy tab keys (`audiences`, `social`, `campaigns`, `forms`, `insights`) mapped to new keys. Removed redundant panels (MarketingLaunchpad, StrategyPanel, MarketingBriefPanel — consolidated into AI hub). Features `useMarketing` hook for state, `useMarketingAiHub` for AI suggestions, cross-module event listeners, post composer, and Lead-to-Revenue Funnel analytics. Page shell ~230 lines.
