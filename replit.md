...
- **AI Command Hub System:** Unified AI access point per module — a single floating button that expands into a full-featured panel with two modes: **Tools** (categorized AI capabilities with execute/result flow) and **Insights** (proactive AI suggestions). Core infrastructure: `use-module-ai.ts` hook (supports `AiTool[]` registration with categories, credit costs, selection requirements, and execute functions), `AiCommandHub` + `AiHubTrigger` components (`components/ai/ai-command-hub.tsx`). CRM is the gold standard: `useCrmAiHub` registers 6 tools (Contact Briefing, Lead Scoring, Conversation Prep, Smart Tags, Churn Detection, Pipeline Analysis) with custom result renderers (`crm-tool-results.tsx`). Hub features: glassmorphism fixed panel, tool result drill-down with back navigation, category grouping, Framer Motion transitions, Escape key handling, and context-aware tool availability (selection-dependent tools auto-disable). Keyboard shortcut: Shift+A toggles hub. Backward compatible — `ModuleAiAssistant` + `AiAssistantTrigger` still work for simpler modules.
- **CRM Sequence Scheduler:** `CrmSequenceSchedulerService` runs every 60s via `setInterval` with overlap guard. Auto-processes due `CrmSequenceEnrollment` records: auto-advances `wait` steps, creates `needs_approval` ContactEvents for email/whatsapp steps, creates high-priority `ContactTask` for call steps. Emits `sequence.step_due` event for notification system.
- **CRM Event → Notification Wiring:** `FlowListener` handles `contact.created`, `contact.updated` (status changes only), `contact.merged`, `contact.deleted`, `contact.imported`, and `sequence.step_due` events — creating user-visible notifications for each.
- **Contact Polling:** Lightweight `GET /contacts/poll` endpoint returns `{ lastUpdatedAt, totalCount }`. Frontend polls every 30s (visibility-aware), compares both timestamp and count, triggers silent refresh on changes.
- **ContactList Ghost ID Cleanup:** `cleanContactListIds` helper in `crm.service.ts` automatically purges deleted contact references from MANUAL lists on read.
...
- **Webhook Dispatcher System:** `WebhookDispatcherService` listens to all `EventEmitter2` events (`contact.created`, `contact.updated`, `contact.merged`, `booking.*`, `invoice.*`, `post.published`) and fires registered webhook URLs with HMAC-SHA256 signatures. Enables Zapier/Make/n8n/custom integrations. Headers: `X-KeyFlow-Event`, `X-KeyFlow-Signature`, `X-KeyFlow-Timestamp`. 10s timeout, `Promise.allSettled` for resilience.
- **Reusable Keyboard Shortcuts System:** `useKeyboardShortcuts` hook with `ShortcutGroup` definitions. Auto-skips when typing in inputs/textareas. Supports `ctrl/meta/shift/alt` modifiers and conditional `when()` guards. CRM implementation: N=new contact, F=focus search, 1-4=switch tabs, R=refresh, B=broadcast, G=guide.
- **Module Event Bus:** Frontend `ModuleEventBus` class (`lib/module-events.ts`) for decoupled cross-module communication. Typed events (contact:created/updated/deleted, commerce:*, booking:*, module:tab_changed). Supports wildcard `*` subscriptions and DOM CustomEvent bridge. React hooks: `useModuleEvent(type, handler)` and `useModuleEmit()`.
