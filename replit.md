...
- **AI-Powered Automation:** Business Autopilot System for archetype inference, revenue model detection, task orchestration, and legal/compliance, including a global AI entry point with route-aware context detection, chat drawer, and suggestion nudges. AI Business Office Foundations layer with Intent → Plan → Execute → Log pipeline, 4-tier governance engine (auto/confirm/approve/admin), BusinessGraphService (cached cross-module snapshot), AiExecutionLogService (full audit trail), IntentParserService (OpenAI function-calling intent extraction), PlannerService (intent → ordered AiPlan with steps), and 14 API endpoints for graph/intent/plans/logs/approvals/governance. AI Copilot UX Shell with Global Copilot Panel (slide-out with Chat/Queue/Activity tabs, Cmd+J shortcut, Brain icon in sidebar rail), AI Control Center settings page (governance mode selector with Advisory/Assisted/Pro Auto/Restricted modes, max auto-execution tier picker, blocked tools/modules display), Execution History viewer (filterable log with stats strip, module/search filters, expandable detail rows), Action Queue (pending approvals with Verification Cards supporting approve/reject/defer actions, tier-colored risk indicators), and full API client layer (`lib/client.ts`) with typed interfaces for all AI Office endpoints.
