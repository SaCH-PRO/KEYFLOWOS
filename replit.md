...
- **Marketing Module:** Features Campaigns, Lead Forms, and Insights tabs with standard CRM patterns. Includes status filters, sort options, and a `MarketingGuide`. Utilizes `CampaignActionQueue` and `FormOptimizationQueue` for automation. Insights tab offers Recharts charts for campaign performance. Integrates AI for NL search, campaign content generation, performance analysis, audience segmentation, subject line optimization, and lead form optimization. Emits events for `campaign` and `lead_form` lifecycle changes.
