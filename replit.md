...
- **Communications Delivery Engine:** Unified outbound publishing system with 6 Prisma models and an adapter-based architecture (META, GOOGLE providers). `DeliveryQueueService` polls every 30s for due deliveries, executes through adapters, and records `DeliveryEvent` audit trail. Full REST API at `/communications/`.
...
- **Commerce Data Layer & Supplier Architecture:** New Prisma models for `SupplierConnection`, `SupplierProduct`, `SupplierVariant`, `ProductVariant`, `ProductSourceLink`, `ProductCostProfile`, `MarginSnapshot`, and `FulfillmentRoute`. Product model upgraded with fulfillmentModel, inventoryMode, compareAtPrice. MarketplaceListing upgraded with fulfillmentStrategy enum. PurchaseOrder links to SupplierConnection. Supplier adapter interface defined. ProductNormalizationService converts supplier products to internal Products. Full CRUD API at /supplier/* and tRPC router at supplier.* namespace. New SupplierModule at apps/server/src/modules/supplier/.
