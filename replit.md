...
- **Pluggable Module AI System:** Reusable AI assistant architecture (`use-module-ai.ts` hook + `ModuleAiAssistant` + `AiAssistantTrigger` components) that any module can drop in for context-aware AI suggestions. CRM is the first implementation via `useCrmAiAssistant`. The pattern: (1) create a module-specific hook that implements `generateSuggestions`, (2) wire `AiAssistantTrigger` into the page header, (3) render `ModuleAiAssistant` panel. Supports suggestion types (action/insight/warning/tip), priority levels, dismissal, and action execution. Files: `hooks/use-module-ai.ts`, `components/ai/module-ai-assistant.tsx`, `crm/pipeline/hooks/use-crm-ai-assistant.ts`.
